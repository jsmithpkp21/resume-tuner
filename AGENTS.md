...
- Makefile recipes that combine `bash -c "..."` or `bash -lc "..."` (double-quoted) with an embedded `$$VAR` / `$${VAR...}` shell-var expansion are prohibited: the recipe shell (`/bin/sh`) expands the value before bash re-parses the command string, allowing quote-breakout injection if the value contains `"` (see PR #424). Enforcement is via `scripts/check_makefile_bash_quoting.py` (run by `make makefile-quoting-check`, also chained into `make lint` so CI gates it). Use single quotes around the `bash -c/-lc` argument so bash performs the expansion at parse time, where the variable's value cannot terminate quoting. Per-line opt-out: `# noqa: bash-lc-quoting` if the shell var is provably safe (e.g. a make-internal value with no user reach).
