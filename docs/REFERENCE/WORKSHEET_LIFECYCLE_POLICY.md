...
- **Canonical coverage** — `python3 scripts/check_canonical_coverage.py --strict`. Reads `experience_db.toml` and `skills_matrix.csv` only, with no worksheet dependence. Hard-gates on canonical invariants from two sources: `DESIGN.md` (rule 11: ≥3 `bullet_bank` entries) and `scripts/validate_experience_data.py` (bullet skills and `related_skills` must exist in `skills_matrix.csv`). The `general_role_description`-non-empty gate is a new invariant this script introduces — DESIGN.md lists "General Role Description" among the canonical experience fields but does not mark it required or non-empty, so this script is the first place to fail-close on emptiness. Soft signals on bullets-per-role distribution and skill-matrix utilization across bullet skills, `related_skills`, and `related_skills_inferred`.
